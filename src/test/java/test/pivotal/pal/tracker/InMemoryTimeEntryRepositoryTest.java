...
public class InMemoryTimeEntryRepositoryTest {
    ...
    public void list() throws Exception {
        InMemoryTimeEntryRepository repo = new InMemoryTimeEntryRepository();
        repo.create(new TimeEntry(123, 456, LocalDate.parse("2017-01-08"), 8));
        repo.create(new TimeEntry(789, 654, LocalDate.parse("2017-01-08"), 4));

        List<TimeEntry> expected = asList(
                new TimeEntry(1L, 123, 456, LocalDate.parse("2017-01-08"), 8),
                new TimeEntry(2L, 789, 654, LocalDate.parse("2017-01-08"), 4)
        );
        assertThat(repo.list().equals(expected));
    }
    ...
    public void update() throws Exception {
        ...
        TimeEntry updatedEntry = repo.update(
                created.getId(),
                new TimeEntry(321, 654, LocalDate.parse("2017-01-08"), 5));

        TimeEntry expected = new TimeEntry(created.getId(), 321, 654,LocalDate.parse("2017-01-08"), 5);
        assertThat(updatedEntry).isEqualToComparingFieldByField(expected);
        assertThat(repo.get(created.getId())).isEqualToComparingFieldByField(expected);
    }
